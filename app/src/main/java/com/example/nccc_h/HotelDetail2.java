...
public class HotelDetail2 extends AppCompatActivity {
    ...
    //평가하기 시크바 초기 설정
    public void setSeek() {
        userVal1 = value1.get(0);
        userVal2 = value2.get(0);
        userVal3 = value3.get(0);
        userVal4 = value4.get(0);
        userVal5 = value5.get(0);
    ...
    public void setFirst() {
        ...
        popTourList.setMovementMethod(new ScrollingMovementMethod());
        nearTourList = (TextView) findViewById(R.id.hotel_near_Tourlist);
        nearTourList.setMovementMethod(new ScrollingMovementMethod());
    }
    ...
    public void getHotelData() {
        ...
        popTourList.setText(pop);
        nearTourList.setText(near2);

        htlReservation.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent i = new Intent(Intent.ACTION_VIEW, Uri.parse(url.get(0)));
                startActivity(i);
            }
        });
    }
    ...
    public void getDetailJsonData() {
        try {
            ...
            pop = popular.get(0).replace("||", "\n");
            near2 = near.get(0).replace("||", "\n");
    ...
    //방문한 호텔의 경우 추가 정보 전달
    class Visited extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... strings) {
            try {
                String str;
                URL url = new URL("http://222.116.135.77:8080/NCCC_H/visitedhotel.jsp");
                HttpURLConnection conn = (HttpURLConnection) url.openConnection();
                conn.setRequestProperty("Content-Type", "application/x-www-form-urlencoded");
                conn.setRequestMethod("POST");
                ...
                sendMsg = "userid="+strings[0]+"&hotelname="+strings[1]+"&hotelcode="+strings[2];
                osw.write(sendMsg);
                osw.flush();
