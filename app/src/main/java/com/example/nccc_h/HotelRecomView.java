...
public class HotelRecomView extends AppCompatActivity {
    ...
    String cityValue; //도시값
    String typeValue; //타입값
    String min, max;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_hotel_recom_view);

        Intent i = getIntent();

        userID = i.getStringExtra("userID");
        cityValue = i.getStringExtra("cityValue");
        typeValue = i.getStringExtra("typeValue");
        ...
        //recyclerView 초기화
        RecyclerView recyclerView = (RecyclerView) findViewById(R.id.hotel_recom_view);
    ...
    public void getHotelJsonData() {
        try {
            String j;
            result = hotelView.execute(userID, cityValue, typeValue, min, max).get();
    ...
    class HotelView extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... strings) {
            try {
                String str;
                URL url = new URL("http://222.116.135.77:8080/NCCC_H/hotelrecomview.jsp");
                ...
                sendMsg = "userid="+strings[0]+"&cityvalue="+strings[1]+"&typevalue="+strings[2]+"&min="+strings[3]+"&max="+strings[4];
                osw.write(sendMsg);
                osw.flush();
